...
exports.pushLambda = function(appName) {

    return new aws.lambda.CallbackFunction(`${appName}-push-lambda`, {
        memorySize: 256 /*MB*/,
        environment: {
        ...
        callback: (event) => {
            ...
            if (device === 'android') {
                ...
                console.log('test', process.env.ECR_REPOSITORY_URI);
...
// exports.pushLambda = function(event) {
//     const message = event.Records[0].body;
//     const token = event.Records[0].messageAttributes.token.stringValue;
//     const device = event.Records[0].messageAttributes.device.stringValue;
