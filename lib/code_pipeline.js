...
exports.createPipeline = function(appName, containerName, service, cluster, environment, db) {
    const buildRole = createBuildRole(appName);
    const {vpc, securityGroups: { dataSg } } = environment;
    ...
    const buildProject = new aws.codebuild.Project(`${appName}-cb`, {
        ...
        // Allow access to RDS space to run migration before release
        vpcConfig: {
        ...
        environment: {
          ...
          environmentVariables: [
                ...
                {name: "PGDATABASE", value: dbName.secretString},
                {name: "PGUSER", value: dbUser.secretString},
                {name: "PGPASSWORD", value: dbPassword.secretString},
                {name: "PGHOST", value: db.address},
            ]
        },
