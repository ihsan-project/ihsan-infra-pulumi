...
exports.createVPC = function(name) {
    ...
    const lbSg = new awsx.ec2.SecurityGroup(`${name}-sg-lb`, { vpc });
    ...
    const appSg = new awsx.ec2.SecurityGroup(`${name}-sg-app`, { vpc });
    ...
    const dataSg = new awsx.ec2.SecurityGroup(`${name}-sg-data`, { vpc });
    // Make the LB security group the only source for the application security group
    ...
    return {vpc, securityGroups: { appSg, lbSg, dataSg }};
}
